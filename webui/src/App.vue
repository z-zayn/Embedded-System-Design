<template>
  <div class="page">
        ...
        <el-card>
          ...
          <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="filesrv">
              ...
              <span class="muted" v-if="ctl.pid">PID: {{ ctl.pid }}</span>
            </el-descriptions-item>
          </el-descriptions>

          <div class="ctl">
            <el-button type="success" @click="startSrv" :disabled="isFilesrvRunning" :loading="ctlBusy">
              Start
            </el-button>
            <el-button type="danger" @click="stopSrv" :disabled="!isFilesrvRunning" :loading="ctlBusy">
              Stop
            </el-button>
          </div>
        </el-card>
        ...
        <el-card>
          <template #header>
            <div class="card-header">
              ...
              <el-button size="small" @click="refreshFiles" :loading="filesLoading">Refresh</el-button>
            </div>
          </template>

          <el-table :data="files" height="360" size="small" stripe>
          ...
          <div class="hint">
            Tip: Stop filesrv and run filecli again to show “upload fails when stopped” in your demo.
          </div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-card style="margin-top:16px;">
      ...
      <el-alert v-if="lastMsg" :title="lastMsg" type="info" show-icon />
...
const memTotalMB = computed(() => Math.round((status.value?.memTotalKb ?? 0) / 1024))
...
.mem-text { color: #666; font-size: 12px; }
.muted { margin-left: 10px; color: #666; }
.ctl { margin-top: 12px; display: flex; gap: 10px; }
.pre { background: #0b1020; color: #d6e1ff; padding: 12px; border-radius: 6px; overflow: auto; }
